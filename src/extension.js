...
const PowerTracker = GObject.registerClass(
  class PowerTracker extends PanelMenu.Button {
    _init(settings) {
      ...
      super._init(0.0, gettext("PowerTracker")); // Use gettext here
...
class PowerTrackerExtension {
  ...
  enable() {
    ...
    this._powertracker = new PowerTracker(this._settings);
    Main.panel.addToStatusArea(this._uuid, this._powertracker);

    this._powertracker.menu.addAction(gettext("Preferences"), () => { // Use gettext here
...
function init(meta) {
  return new PowerTrackerExtension(meta.uuid);
}